...
import {
  ...
  Shield, Lock, Save, Loader2, Crown, Key
} from 'lucide-react';
...
export default function HODProfile() {
  ...
  useEffect(() => {
    ...
    if (currentUser?.avatar && !avatarPreview) {
      ...
      const storageKey = currentUser?.id
        ? `user_avatar_${currentUser.id}`
        : currentUser?.email
          ? `user_avatar_${currentUser.email}`
          : null;
  ...
  }, [currentUser?.avatar, avatarPreview, currentUser?.id, currentUser?.email]);
          ...
          if (uploadResponse.success) {
            ...
            const storageKey = currentUser?.id
              ...
              : currentUser?.email
                ...
                : null;
            if (storageKey) {
              localStorage.setItem(storageKey, newAvatarUrl);
            }
          } else {
  ...
  const departmentName = department?.name
    || (typeof (currentUser as { department?: { name?: string } }).department?.name === 'string'
      ? (currentUser as { department?: { name?: string } }).department?.name
      : undefined)
    || (typeof (currentUser as { department?: string }).department === 'string'
      ? (currentUser as { department?: string }).department
      : undefined)
    || 'Department';
                ...
                <div className="px-6 pt-8 pb-6 text-center relative">
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                    <div className="text-center">
                      ...
                      <p className="font-semibold text-slate-700 dark:text-slate-200 mt-1">HOD</p>
                    </div>
