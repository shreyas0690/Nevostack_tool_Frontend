...
export default function SaaSSuperAdminHeader({ onTabChange }: SaaSSuperAdminHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md supports-[backdrop-filter]:bg-white/80 dark:supports-[backdrop-filter]:bg-slate-900/80 shadow-sm">
      <div className="px-3 sm:px-6 py-2 flex items-center gap-2 sm:gap-4">
        {/* Brand */}
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-shrink-0">
          <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 rounded-xl overflow-hidden">
            <img 
              src="/nevolog.png" 
              alt="NevoStack Logo" 
          ...
          <div className="hidden sm:block">
            <h1 className="text-lg sm:text-xl md:text-3xl font-black leading-none bg-gradient-to-r from-blue-600 via-purple-600 to-blue-600 dark:from-blue-400 dark:via-purple-400 dark:to-blue-400 bg-clip-text text-transparent tracking-tighter drop-shadow-md">
              NevoStack Platform
            </h1>
        ...
        <div className="flex-1 hidden lg:flex justify-center">
          <div className="relative w-full max-w-xl">
            ...
            <Input
              ...
              className="pl-10 pr-4 py-2 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 shadow-sm hover:shadow-md"
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="hidden sm:inline-flex h-9 w-9 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 hover:scale-105">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" aria-label="Toggle theme" className="h-9 w-9 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 hover:scale-105">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden lg:flex items-center gap-2">
              <div className="text-right">
                <p className="font-semibold text-sm truncate max-w-[160px] text-slate-800 dark:text-slate-200">{currentUser ? `${currentUser.firstName || ''} ${currentUser.lastName || ''}`.trim() : ''}</p>
                <div className="flex items-center gap-1.5">
                  <Badge variant={getRoleBadgeVariant(currentUser?.role || '')} className="text-xs px-2 py-0.5">
                    {formatRole(currentUser?.role || '')}
                  </Badge>
                  <Badge variant="outline" className="text-xs px-2 py-0.5 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-blue-200 dark:border-blue-700">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="p-0 w-8 h-8 sm:w-9 sm:h-9 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 hover:scale-105">
