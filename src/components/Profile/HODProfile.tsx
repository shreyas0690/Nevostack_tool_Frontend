...
import {
  ...
  Shield, Lock, Save, Loader2, Crown
} from 'lucide-react';
...
export default function HODProfile() {
  ...
  useEffect(() => {
    console.log('🔍 HODProfile - currentUser:', currentUser);
    console.log('🔍 HODProfile - currentUser.name:', currentUser?.name);
    console.log('🔍 HODProfile - currentUser.firstName:', currentUser?.firstName);
    console.log('🔍 HODProfile - currentUser.lastName:', currentUser?.lastName);
    console.log('🔍 HODProfile - firstName state:', firstName);
    console.log('🔍 HODProfile - lastName state:', lastName);
    console.log('🔍 HODProfile - avatarPreview:', avatarPreview);
    console.log('🔍 HODProfile - initials:', initials);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      // Update profile data via HOD panel backend
      console.log('Updating HOD profile data...');
      const updateResponse = await userService.updateHODProfileDetails({
        firstName,
        lastName
      });

      if (updateResponse.success) {
        console.log('HOD profile data updated successfully');
        ...
        if (currentUser && updateCurrentUser) {
          ...
          console.log('✅ Updated currentUser with new HOD profile data');
        }
      } else {
        throw new Error(updateResponse.message || 'Profile update failed');
      }

      // Upload Avatar to HOD panel backend if selected
      if (avatarFile) {
        try {
          console.log('Uploading HOD avatar...');
          const uploadResponse = await userService.uploadHODAvatar(avatarFile);
          if (uploadResponse.success) {
            // Update the avatar URL from backend response
            const newAvatarUrl = uploadResponse.data.avatar;
            setAvatarPreview(newAvatarUrl);
            console.log('HOD avatar uploaded successfully:', newAvatarUrl);
            ...
            if (currentUser && updateCurrentUser) {
              ...
              console.log('🔄 About to call updateCurrentUser with:', updatedUser);
              updateCurrentUser(updatedUser);
              console.log('✅ Updated currentUser with new HOD avatar');
            }
          } else {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-700 text-white p-8 shadow-xl">
        ...
        <div>
              <h1 className="text-4xl font-bold tracking-tight">HOD Profile</h1>
              <p className="text-purple-100 text-lg mt-1">Manage your department head profile and preferences</p>
            </div>
          </div>
          <div className="flex items-center gap-6 mt-6">
            ...
            <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-lg backdrop-blur-sm">
              ...
              <span className="text-sm font-medium capitalize">Head of Department</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 bg-gradient-to-r from-purple-100 to-indigo-100 dark:from-purple-900/30 dark:to-indigo-900/30 rounded-xl flex items-center justify-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="lg:col-span-1 border-0 shadow-lg bg-gradient-to-br from-white via-purple-50/50 to-indigo-50/50 dark:from-gray-900 dark:via-purple-950/20 dark:to-indigo-950/20">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Button
                    ...
                    size="sm"
                    disabled={isSaving}
                    className="w-full border-purple-200 hover:bg-purple-50 hover:border-purple-300 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-11 border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
                  />
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
              ...
              <div className="flex justify-end pt-4 border-t">
                <Button
                  ...
                  disabled={isSaving}
                  className="h-11 px-8 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSaving ? (
                    <>
