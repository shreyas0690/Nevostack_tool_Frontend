...
import {
  ...
  Shield, Lock, Save, Loader2, Users
} from 'lucide-react';
...
export default function HRProfile() {
  ...
  useEffect(() => {
    console.log('🔍 HRProfile - currentUser:', currentUser);
    console.log('🔍 HRProfile - currentUser.name:', currentUser?.name);
    console.log('🔍 HRProfile - currentUser.firstName:', currentUser?.firstName);
    console.log('🔍 HRProfile - currentUser.lastName:', currentUser?.lastName);
    console.log('🔍 HRProfile - firstName state:', firstName);
    console.log('🔍 HRProfile - lastName state:', lastName);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      // Update profile data via HR panel backend
      console.log('Updating HR profile data...');
      const updateResponse = await userService.updateHRProfileDetails({
        firstName,
        lastName
      });

      if (updateResponse.success) {
        console.log('HR profile data updated successfully');
        ...
        if (currentUser && updateCurrentUser) {
          ...
          console.log('✅ Updated currentUser with new HR profile data');
        }
      } else {
        throw new Error(updateResponse.message || 'Profile update failed');
      }

      // Upload Avatar to HR panel backend if selected
      if (avatarFile) {
        try {
          console.log('Uploading HR avatar...');
          const uploadResponse = await userService.uploadHRAvatar(avatarFile);
          if (uploadResponse.success) {
            // Update the avatar URL from backend response
            const newAvatarUrl = uploadResponse.data.avatar;
            setAvatarPreview(newAvatarUrl);
            console.log('HR avatar uploaded successfully:', newAvatarUrl);
            ...
            if (currentUser && updateCurrentUser) {
              ...
              console.log('🔄 About to call updateCurrentUser with:', updatedUser);
              updateCurrentUser(updatedUser);
              console.log('✅ Updated currentUser with new HR avatar');
            }
          } else {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700 text-white p-8 shadow-xl">
        ...
        <div className="relative">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-4xl font-bold tracking-tight">HR Profile</h1>
              <p className="text-blue-100 text-lg mt-1">Manage your human resources profile and preferences</p>
            </div>
          </div>
          <div className="flex items-center gap-6 mt-6">
            ...
            <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-lg backdrop-blur-sm">
              ...
              <span className="text-sm font-medium capitalize">Human Resources</span>
            </div>
          </div>
        </div>
      </div>
