...
export default function HODHeader({ onNavigate, onMenuToggle }: HODHeaderProps) {
  ...
  console.log('🚀 HODHeader render - currentUser:', currentUser, 'headerUser:', headerUser, 'name:', headerUser?.name, 'loading:', loading, 'renderKey:', renderKey);

  // Sync headerUser with currentUser when it changes
  React.useEffect(() => {
    if (currentUser && currentUser !== headerUser) {
      console.log('🔄 HODHeader - Syncing headerUser with currentUser:', currentUser);
  ...
  React.useEffect(() => {
    console.log('🔍 HODHeader - displayUser:', displayUser);
    console.log('🔍 HODHeader - displayUser.avatar:', displayUser?.avatar);
    console.log('🔍 HODHeader - avatarKey:', avatarKey);
    if (displayUser?.avatar) {
      setAvatarKey(prev => prev + 1);
      console.log('🔄 HODHeader - Avatar key updated to force re-render');
    } else {
      console.log('ℹ️ HODHeader - No avatar, showing initials');
    }
  }, [displayUser?.avatar]);

  // Additional debug: Log any displayUser changes
  React.useEffect(() => {
    console.log('👤 HODHeader - displayUser changed:', displayUser);
    console.log('👤 HODHeader - displayUser.name:', displayUser?.name);
    console.log('👤 HODHeader - displayUser.firstName:', displayUser?.firstName);
    console.log('👤 HODHeader - displayUser.lastName:', displayUser?.lastName);
    console.log('👤 HODHeader - isAuthenticated:', true); // Since we're in HOD header
  ...
  const getDisplayBranding = () => {
    ...
    if (companyData && companyData.name) {
      return {
        name: `${companyData.name} - HOD Panel`,
        shortName: companyData.name,
        tagline: 'Department Management',
        ...
        <div className="ml-auto flex items-center gap-1 md:gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-72">
              <DropdownMenuLabel>{branding.shortName} HOD Help & Support</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuGroup>
                <DropdownMenuItem onClick={() => window.open('https://nevostack.com/docs/hod', '_blank')}>
                ...
                <DropdownMenuItem onClick={() => window.open('mailto:support@nevostack.com?subject=HOD%20Support%20Request', '_blank')}>
                  ...
                  <Avatar>
                    {displayUser?.avatar ? (
                      <img
                        ...
                        onLoad={() => console.log('✅ HOD Header Avatar loaded successfully:', displayUser.avatar)}
                        onError={(e) => {
                          console.error('❌ HOD Header Avatar failed to load:', displayUser.avatar);
                  ...
                  <div className="flex items-center gap-3">
                    <Avatar className="w-12 h-12">
                      {displayUser?.avatar ? (
                        ...
                        <AvatarFallback className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white text-sm">
                          {avatarInitials}
                        </AvatarFallback>
                      )}
                    </Avatar>
