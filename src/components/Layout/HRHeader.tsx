...
export default function HRHeader({ onNavigate }: HRHeaderProps) {
  ...
  console.log('🚀 HRHeader render - currentUser:', currentUser, 'headerUser:', headerUser, 'name:', headerUser?.name, 'loading:', loading, 'renderKey:', renderKey);

  // Sync headerUser with currentUser when it changes
  React.useEffect(() => {
    if (currentUser && currentUser !== headerUser) {
      console.log('🔄 HRHeader - Syncing headerUser with currentUser:', currentUser);
  ...
  React.useEffect(() => {
    console.log('🔍 HRHeader - displayUser:', displayUser);
    console.log('🔍 HRHeader - displayUser.avatar:', displayUser?.avatar);
    console.log('🔍 HRHeader - avatarKey:', avatarKey);
    if (displayUser?.avatar) {
      setAvatarKey(prev => prev + 1);
      console.log('🔄 HRHeader - Avatar key updated to force re-render');
    } else {
      console.log('ℹ️ HRHeader - No avatar, showing initials');
    }
  }, [displayUser?.avatar]);

  // Additional debug: Log any displayUser changes
  React.useEffect(() => {
    console.log('👤 HRHeader - displayUser changed:', displayUser);
    console.log('👤 HRHeader - displayUser.name:', displayUser?.name);
    console.log('👤 HRHeader - displayUser.firstName:', displayUser?.firstName);
    console.log('👤 HRHeader - displayUser.lastName:', displayUser?.lastName);
    console.log('👤 HRHeader - isAuthenticated:', true); // Since we're in HR header
  ...
  const avatarInitials = (() => {
    ...
    if (!name) return 'HR';
    return name.split(' ').filter(Boolean).map(n => n[0]).join('').slice(0,2).toUpperCase() || 'HR';
  })();

  // Find user's department
  const userDepartment = displayUser?.departmentId ? 
    mockDepartments.find(d => d.id === displayUser.departmentId) : null;

  const copyUserId = async () => {
    try {
      await navigator.clipboard.writeText(displayUser?.id || '');
      ...
      <div className="px-6 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="hidden md:block">
            <h1 className="text-xl font-bold leading-tight">
              {branding.shortName} - HR Panel
            </h1>
            <p className="text-xs text-muted-foreground">
              {userDepartment?.name || 'HR Department'} Management
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 hidden md:flex justify-center">
          <div className="relative w-full max-w-xl">
            ...
            <Input
              type="search"
              placeholder={`Search ${userDepartment?.name || 'employees, departments'}...`}
              className="pl-9"
        ...
        <div className="ml-auto flex items-center gap-1 md:gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-72">
              <DropdownMenuLabel>{branding.shortName} HR Help & Support</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuGroup>
                <DropdownMenuItem onClick={() => window.open('https://nevostack.com/docs/hr', '_blank')}>
                ...
                <DropdownMenuItem onClick={() => window.open('mailto:support@nevostack.com?subject=HR%20Support%20Request', '_blank')}>
                  ...
                  <Avatar>
                    {displayUser?.avatar ? (
                      <img
                        ...
                        onLoad={() => console.log('✅ HR Header Avatar loaded successfully:', displayUser.avatar)}
                        onError={(e) => {
                          console.error('❌ HR Header Avatar failed to load:', displayUser.avatar);
