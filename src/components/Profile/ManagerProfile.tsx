...
export default function ManagerProfile() {
  ...
  useEffect(() => {
    console.log('🔍 ManagerProfile - currentUser:', currentUser);
    console.log('🔍 ManagerProfile - currentUser.name:', currentUser?.name);
    console.log('🔍 ManagerProfile - currentUser.firstName:', currentUser?.firstName);
    console.log('🔍 ManagerProfile - currentUser.lastName:', currentUser?.lastName);
    console.log('🔍 ManagerProfile - firstName state:', firstName);
    console.log('🔍 ManagerProfile - lastName state:', lastName);
    console.log('🔍 ManagerProfile - avatarPreview:', avatarPreview);
    console.log('🔍 ManagerProfile - initials:', initials);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      // Update profile data via manager panel backend
      console.log('Updating manager profile data...');
      const updateResponse = await userService.updateManagerProfileDetails({
        firstName,
        lastName
      });

      if (updateResponse.success) {
        console.log('Manager profile data updated successfully');
        ...
        if (currentUser && updateCurrentUser) {
          ...
          console.log('✅ Updated currentUser with new manager profile data');
        }
      } else {
        throw new Error(updateResponse.message || 'Profile update failed');
      }

      // Upload Avatar to manager panel backend if selected
      if (avatarFile) {
        try {
          console.log('Uploading manager avatar...');
          const uploadResponse = await userService.uploadManagerAvatar(avatarFile);
          if (uploadResponse.success) {
            // Update the avatar URL from backend response
            const newAvatarUrl = uploadResponse.data.avatar;
            setAvatarPreview(newAvatarUrl);
            console.log('Manager avatar uploaded successfully:', newAvatarUrl);
            ...
            if (currentUser && updateCurrentUser) {
              ...
              console.log('🔄 About to call updateCurrentUser with:', updatedUser);
              updateCurrentUser(updatedUser);
              console.log('✅ Updated currentUser with new manager avatar');
            }
          } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-700 text-white p-8 shadow-xl">
        ...
        <div>
              <h1 className="text-4xl font-bold tracking-tight">Manager Profile</h1>
              <p className="text-purple-100 text-lg mt-1">Manage your manager profile and preferences</p>
            </div>
          </div>
